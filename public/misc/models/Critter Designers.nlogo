...
_GRASS-GROWTH_ controls how fast the grass grows back on each patch.
...
_CHANGE SHAPE_ randomly changes the shape of the critters you are going to release. The newly assigned shape will show up in the _YOUR CRITTER SHAPE_ monitor.
...
**Client Switches:**
_GRAY-OUT-OTHERS?_ controls whether to display the colors of all the species for the client (set the value to "On") or keep the color of client's species visible and turning all other species gray (set the value to "Off").

_FOLLOW-A-CRITTER?_ controls whether the client's view remains looking at the entire ecosystem (set the value to "Off") or attaches to a single randomly selected critter (set the value of "On").

_SHOW-ENERGY?_ displays the energy level for each individual critter of your species when set to "On".

**Client Monitors:**
_YOUR CURRENT SPECIES LONGEVITY_ reports how long the current species has existed (elapsed time) in the ecosystem.
...
_ALIVE SPECIES_ reports how many species are currently in this ecosystem.

_EXTINCT SPECIES_ reports how many species have died out completely in this ecosystem.
