...
object DecryptionUtil {
  ...
  def decodeForHubNet(encryptedStr: String, isTeacher: Boolean) : Validation[String, HubNetSettings] = {
    ...
    HubNetSettings(decrypteds, isTeacher) map (Success(_)) getOrElse Failure("Failed to interpret input")
  }
