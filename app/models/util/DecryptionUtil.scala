...
object DecryptionUtil {
  ...
  def decodeForHubNet(encryptedStr: String) : Validation[String, HubNetSettings] = {
    ...
    HubNetSettings(decrypteds) map (Success(_)) getOrElse Failure("Failed to interpret input")
  }
